// 時間帯設定データ（フロントエンド用）
// GAS側の TimeSlotConfig.gs と同等のデータ
...
export function getTimeslotTime(group, day, timeslot) {
	try {
...
export function getTimeslotDisplayName(group, day, timeslot) {
	const time = getTimeslotTime(group, day, timeslot);
	return `${timeslot}時間帯${time ? ` (${time})` : ''}`;
}
